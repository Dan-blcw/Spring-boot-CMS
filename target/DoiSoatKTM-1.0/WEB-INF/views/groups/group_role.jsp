                            ...
                            <div class="box-body">
                                ...
                                <div class="row">
                                    <div class="form-group col-sm-12">
                                        <label for="roles" class="col-sm-3 control-label">Quyền: </label>
                                        <div class="col-sm-8">
                                            ...
                                            <c:forEach items="${listRoles2}" var="obj" varStatus="varStatus">
                                                <div class="checkbox checkbox-primary col-sm-4">
                                                    <c:choose>
                                                        <c:when test="${obj.assignGroup == 'true'}">
                                                            <input id="${obj.id}" class="styled checkRole chckHead" type="checkbox" checked="checked" >
                                                        </c:when> 
                                                        <c:otherwise>
                                                            <input id="${obj.id}" class="styled checkRole chckHead" type="checkbox">
                                                        </c:otherwise>
                                                    </c:choose>
                                                    <label for="${obj.id}">
                                                        <b> ${obj.description}</b>
                                                    </label>
                                                    <c:forEach items="${obj.listRoles}" var="obj2" varStatus="varStatus">
                                                        <div class="checkbox checkbox-primary col-sm-12">
                                                            <c:choose>
                                                                <c:when test="${obj2.assignGroup == 'true'}">
                                                                    <input id="${obj2.id}" class="styled checkRole chcktbl" type="checkbox" checked="checked" >
                                                                </c:when> 
                                                                <c:otherwise>
                                                                    <input id="${obj2.id}" class="styled checkRole chcktbl" type="checkbox">
                                                                </c:otherwise>
                                                            </c:choose>
                                                            <label for="${obj2.id}">
                                                                <i>  ${obj2.description}</i>
                                                            </label>
                                                        </div>
                                                    </c:forEach>
                                                </div>
...
<script>
    ...
    $('.chckHead').on("change", function () {
        $(this).closest('.checkbox').find(".chcktbl").prop('checked', $(this).prop('checked'));
    });
    ...
    $('.checkRole').change(function () {
        ...
        if ($('.checkRole:checked').length === $('.checkRole').length) {
            //console.log("OK4");
    ...
    function editInfoViaAjax() {
        ...
        $(".checkRole:checked:not(:disabled)").each(function () {
            listRoles.push({id: $(this).attr('id')});
        });

        edit["id"] = $("#viewForm-id").val().trim();
        edit["name"] = $("#viewForm-name").val().trim();
        edit["listRoles"] = listRoles;

        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: "${pageContext.request.contextPath}/groups/updateListRole",
